...
func NewArchiveStreamWriterMap(mounts []types.MountPoint, dest string) *ArchiveStreamWriterMap {
	...
	for _, m := range mounts {
		...
		aw.filterSpec = vicarchive.GenerateFilterSpec(dest, aw.mountPoint.Destination, !strings.Contains(aw.mountPoint.Destination, dest), vicarchive.CopyTo)
...
func NewArchiveStreamReaderMap(mounts []types.MountPoint, dest string) *ArchiveStreamReaderMap {
	...
	for _, m := range mounts {
		...
		ar.filterSpec = vicarchive.GenerateFilterSpec(dest, ar.mountPoint.Destination, !strings.Contains(ar.mountPoint.Destination, dest), vicarchive.CopyFrom)
